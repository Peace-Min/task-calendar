...
## 5. 자동 업데이트 켜기 ⚠ (이 릴리스는 기본 휴면)

**이 0.9.0 릴리스는 `DefaultUpdateSourceUrl = ""`(빈 값 = 자동 업데이트 휴면)로 나간다.** (`widget/MainWindow.xaml.cs:44`)
즉 **공유폴더에 파일을 올려도, 각 PC에서 소스 URL을 넣기 전까진 위젯이 확인하러 가지 않는다.**

켜는 방법은 두 가지 — 하나만 하면 된다.
