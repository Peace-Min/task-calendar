...
실행 중인 위젯(CDP 9222, **관리자 계정으로 로그인**)과 실 DB 로 `docs/USER-ADMIN.md` 의 편집 경로를 한 라운드에 16 케이스로 돈다.
★ 2026-09-10 사용자 결정으로 조작 화면이 **「구성원 편집」(`openUserAdmin()` / `#userAdminModal`)** 으로 옮겨 갔다 —
편집 케이스는 전부 그 경로로 돌고, 「구성원 보기」는 따로 C16 이 본다.
...
C13 비관리자 뷰(로그인 계정을 잠시 editor 로 내려 **진입 버튼 부재 + 편집 화면 컨트롤 0 + 편집 폼 진입 불가 + 관문 거부** 확인 후 **반드시 복원**) ·
C16 「구성원 보기」는 **관리자에게도** 편집 컨트롤이 0 · 재진입 가드. 케이스마다 불변식 I1~I5
...
C06 알 수 없는 대상/이미 없는 항목 · C07 자기 계정 · C08 비관리자(로그인 계정을 잠시 `editor` 로 내려 **`#uaTrash` 부재 + `admin:false` +
목록 미탑재 + 관문 거부** 확인 후 **반드시 복원**. 활성 관리자가 0 이 되지 않도록 대역 관리자(zzU)를 먼저 세우고 끝에 치운다).
진입 문은 **둘**이다(2026-09-18 · §5.0): 과제·발주처·구분·상태는 「공식 과제 (DB)」 하단 줄의 `#offTrash`(위젯에서만 노출) → `openTrash('project')`,
퇴사자는 「구성원 편집」 상단 막대의 `#uaTrash`(관리자에게만 생성) → `openTrash('user')`. 「사용자 정보」에는 휴지통이 없다.
