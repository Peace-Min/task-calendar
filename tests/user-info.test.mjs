...
test('변이㉔: 구성원 조회가 쓰기 관문(OpenWriteAsync)을 쓰면 membersHostReadPath 가 실패한다', () => {
  const bad = mutateInMember(pdb, 'public async Task<string?> LoadMembersJsonAsync(string loginId, bool includeInactive = false, bool flatOrder = false)',
    'await using var conn = await OpenReadAsync(cts.Token);', 'await using var conn = await OpenWriteAsync(cts.Token);');
...
test('변이㊲-e: 조회 뒤 applyUser 로 세션을 갈아끼우면 identityLiveFromInfo 가 실패한다(§2.5)', () => {
  //  ★ 앵커가 제자리로 돌아왔다(2026-09-10 → 2026-09-18): 이 자리에 끼어들었던 usAdminBtnSync
  //    호출이 사라져(USER-ADMIN §11-38) 다시 "set('usPermMsg','') 바로 다음이 return" 이다.
