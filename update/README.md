...
[각 위젯] 시작(~9s)+30분마다 백그라운드로 latest.json 확인
   · 접근 실패/최신 → 무음(배너 없음)      · json.version > 내 버전 → 상단 배너
     배너: 🆕 새 버전 X 있습니다  [변경내역]  [업데이트]
       └ [업데이트] → 인스톨러 로컬 임시복사 → sha256 검증 → /SILENT 설치 → 재시작 (사용자 데이터 무관 — 위젯 exe만 교체, 데이터는 서버 DB)
```

> **(2026-09-02) 아래 절은 2026-07-15 v0.7→v0.9 전환 당시 기록이다.** 지금 릴리스 라인은 **0.18.0**(feat/db-app)이고 업데이트 소스 기본값(`DeployConfig.UpdateSourceUrl`)은 **비어 있다** — 저장소에 IP를 박지 않고, 배포 시 공유폴더 URL을 채운다. 개인 PC FTP(192.168.1.175)는 임시 소스였고 서버 공유폴더로 옮기는 순서는 [docs/ROADMAP.md](../docs/ROADMAP.md) §2-3. "현재 배포판" 같은 표현은 그날 기준으로만 읽을 것.
