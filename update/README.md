...
> **(2026-09-02) 아래 절은 2026-07-15 v0.7→v0.9 전환 당시 기록이다.** 지금 릴리스 라인은 **0.18.1**(feat/db-app)이고 업데이트 소스 기본값(`DeployConfig.UpdateSourceUrl`)은 **비어 있다** — 저장소에 IP를 박지 않고, 배포 시 공유폴더 URL을 채운다. 개인 PC FTP(192.168.1.175)는 임시 소스였고 서버 공유폴더로 옮기는 순서는 [docs/ROADMAP.md](../docs/ROADMAP.md) §2-3. "현재 배포판" 같은 표현은 그날 기준으로만 읽을 것.

## ⚠ 부트스트랩 — 딱 한 번은 수동
자동 업데이트는 **그 기능이 이미 든 버전(v0.8.0+)** 에만 신호를 밀 수 있다. 지금 배포된 v0.7.x는 자동 업데이트를 모른다.
1. **최초 1회는 현재 릴리스 인스톨러를 수동 설치** — 동료에게 전달해 한 번 실행하게 한다.
2. 자동 업데이트 소스: 저장소의 `DeployConfig.UpdateSourceUrl`은 **비어 있다**(= 기능 휴면). 배포 빌드 전에 공유폴더 URL로 채우거나, 각 PC 설정의 '소스 URL'에서 지정한다. (끄려면 소스 URL 비움.)
...
> **🔒 ACL — 공유 위치를 만들 때 함께 건다(P1-7, 필수).** 위젯은 이 위치의 exe를 **자동 실행**하므로
> 접근 권한을 **모두 읽기 / 배포자만 쓰기**로 설정한다. 아무나 쓸 수 있으면 exe와 sha256을 함께 바꿔치기해
> 전 PC가 그것을 설치한다 — 해시는 **무결성**(손상 감지)이지 **출처**(누가 올렸나)를 보장하지 않는다.
> UNC면 폴더 NTFS 권한(배포자 계정만 수정), FTP면 업로드 계정 분리. 근거·대안(코드 서명)은 아래 §보안.
...
- 설정·로컬 파일은 업데이트가 건드리지 않음(exe만 교체). **캘린더 데이터는 서버 DB**라 업데이트와 무관.
