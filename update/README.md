...
2. 각 PC 설정 → **자동 업데이트 → 소스 URL** 에 공유폴더 주소 입력. (v0.9.0은 `DefaultUpdateSourceUrl`이 **비어 있음=자동 업데이트 휴면**으로 배포되므로, 소스 URL을 넣어야 자동 업데이트가 켜진다. 전 PC 공통 자동 적용하려면 최종 FTP 확정 후 상수에 박아 재빌드.)
