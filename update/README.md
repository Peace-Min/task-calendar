...
[각 위젯] 시작(~9s)+6시간마다 백그라운드로 latest.json 확인
...
1. **v0.8.1 인스톨러(현재 최신)를 마지막으로 수동 배포**(동료에게 직접 전달·설치).
2. 각 PC 설정 → **자동 업데이트 → 소스 URL** 입력. (v0.8.1부터 `DefaultUpdateSourceUrl = ftp://192.168.1.175/` 가 기본 박혀 있어 별도 설정 없이도 그 FTP를 확인함 — 배포 폴더가 하위경로면 설정에서 변경.)
