...
namespace TaskCalendarWidget
{
    ...
    //   · `data.xml` 에 **남기지 않는다**: 2026-09-01 에 XML 저장 경로를 폐기했다
    //     (DeployConfig.XmlRetired = true). 앱은 그 파일을 읽지도 쓰지도 않는다 —
    //     거기 남겼다면 로컬 유지가 아니라 **동결**이 됐을 것이다.
    //     사용자 PC 의 옛 파일은 「XML 가져오기」로 한 번 읽히고 그것으로 끝난다.
    //
