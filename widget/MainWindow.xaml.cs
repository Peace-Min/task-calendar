...
namespace TaskCalendarWidget
{
    public partial class MainWindow : Window, INetcusHost
    {
        ...
        private void OnWebMessage(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                switch (cmdEl.GetString())
                {
                    ...
                    {
                        string rid = doc.RootElement.TryGetProperty("reqId", out var sReq) ? (sReq.GetString() ?? "") : "";
                        string sj  = doc.RootElement.TryGetProperty("state", out var sSt) ? sSt.GetRawText() : "";
        ...
        //  ★ 저장에 성공할 때마다 갱신한다. 안 하면 두 번째 저장이 낡은 토큰을 보내 전부 충돌한다.
        //  ★ 번호는 재사용되므로 삭제된 항목의 번호가 남아 있으면 나중에 '다른 행'을 가리킨다(H-1) —
        //    그래서 통째로 교체한다(부분 갱신 금지).
        private CalendarSnapshot? _calSnap;
        ...
        //  DB 모드 저장 — 웹이 상태 전체를 넘기면 CalendarWriteDb 가 차이만 쓴다(§3.1~3.4).
