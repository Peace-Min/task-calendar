...
「사용자 정보」에는 휴지통이 없다. `usAdminBtnSync(editRole)` 가 그 「관리」 줄에 만드는 것은 이제 캡션 「관리자」 + 「구성원 편집」(`#usUserAdmin`) **하나**뿐이고, 관리자에서 내려가면 그 줄을 **통째로 비운다**(§11-32·§11-33 · [USER-ADMIN §5.0](USER-ADMIN.md)).
