...
| 진입 | 「사용자 정보」의 **「관리」 줄**(`#usAdminBtns`)에 있는 버튼 `#usTrash` — `edit_role='admin'` 회신일 때만 DOM 에 만든다(`#usUserAdmin` 과 같은 줄·같은 규칙: 숨김이 아니라 부재). 그 줄은 「구성원 보기」 줄과 **분리돼 있고** 캡션 「관리자」로 시작한다 | USER-ADMIN §5.0·§8-7(c) 를 그대로 잇는다. 새 규칙을 만들지 않는다. 줄을 가른 이유는 「휴지통」이 '구성원' 안내문에 딸려 **「퇴사자 휴지통」으로 읽히던** 것을 끊기 위해서다(§11-32) |
...
### 5.0 진입 — 「사용자 정보」의 「관리」 줄, 두 번째 버튼

`#usAdminBtns`(관리 줄)에 `#usTrash`「휴지통」. `usAdminBtnSync(editRole)` 가 `#usUserAdmin` 을 만들 때 **같은 줄·같은 자리에서 함께** 만들고 함께 없앤다(한 함수, 한 판정 문자열). 마크업에는 없다 — 그 줄 자체가 마크업에서 빈 `<div>` 고, 캡션 「관리자」까지 이 함수가 만든다. 관리자에서 내려가면 그 줄을 **통째로 비운다**(§11-32 · [USER-ADMIN §5.0](USER-ADMIN.md)).
