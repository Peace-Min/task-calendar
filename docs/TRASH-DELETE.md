...
| 진입 | 진입은 **둘**이고 둘 다 그 도메인의 화면에 있다 — 과제·발주처·구분·상태는 「공식 과제 (DB)」 화면 하단 줄의 `#offTrash`「🗑 휴지통」(위젯에서만 노출, 「＋ 발주처 관리」와 같은 규칙; 관리자 판정은 열 때 `trashGet` 회신이 한다), 인력은 「구성원 편집」 상단 막대의 `#uaTrash`「퇴사자 휴지통」(관리자·비순서편집일 때만 `uaAdminBar` 가 만든다 — 숨김이 아니라 부재). 「사용자 정보」에는 휴지통이 없다 | 2026-09-18 사용자 결정: 과제 관련 기능은 과제 관리 쪽 팝업에서, 구성원 관련은 사용자 정보 → 구성원 편집 이후에서 전부 해결한다. 숨기는 곳이 곧 되돌리는 곳이고, §11-32 가 잡았던 '「퇴사자 휴지통」으로 읽힌다' 는 오해는 그 이름의 문을 **실제로 구성원 쪽에** 둠으로써 사라진다(§11-33) |
...
| 문 | 어디 | 생기는 조건 | 호출 |
|---|---|---|---|
| `#offTrash`「🗑 휴지통」 | 「공식 과제 (DB)」 화면(`#officialModal`) 하단 줄 — 「지금 새로고침」 옆(도구줄은 600px 폭에서 이미 꽉 차 버튼 하나만 더 서도 두 줄로 접힌다 · loop-ui-visual V3) | **위젯에서만** 보인다(`offSyncExportBtn` 이 `HOST` 로 `style.display` 를 정한다 · 발주처 관리와 같은 규칙). 누르면 `offEditGuard`(위젯·온라인)를 지난다 | `openTrash('project')` |
