...
| 소속 조작 | 추가(**상위 조직 필수 선택**) · 개명 · 숨김/복구 · **형제 안 순서**(▲▼) · **상위 변경**(이 관리창에서 · 순환 금지) | 사용자(2026-09-18): *"SW 5팀·6팀 같은 팀 추가는 가능해야 한다"* + *"소속 편집은 별도 관리창에서 하는 것이 맞다 — 직원 정보 수정에서 바꾸는 것보다."* 조직 개편은 드물지만, 드물 때도 **조직 쪽에서 한 번**에 끝나야지 직원을 한 명씩 열어 소속을 옮기는 일이 되면 안 된다. 상위 변경은 `parent_id` 한 컬럼 UPDATE — 소속자는 번호로 매달려 있어 따라온다. 최상위는 하나뿐(`02-seed-org.sql` 가드가 루트 1개를 요구) — 상위 없는 추가·최상위로의 이동은 받지 않는다 |
...
- **상위 변경은 관리창의 일**(§2): 직원 정보 수정 폼은 사람 한 명의 소속을 **고르는** 곳이고, 조직 자체(이름·상위·순서·숨김)는 이 관리창이 고친다. 팀을 다른 본부로 옮기면 그 팀 소속자 전원이 번호로 따라온다 — 직원을 한 명씩 열 일이 없다.
- **순환 금지**: 상위 변경 시 호스트가 새 상위의 조상 사슬을 걸어 올라가며 대상 자신이 나오면 거부한다(12행이라 메모리 순회). 자기 자신·최상위(NULL)로의 이동도 거부(루트는 하나).
- 추가 시 `parentId` 는 **필수**이고 활성 조직이어야 한다(루트는 시드의 「기술개발총괄」 하나).
...
| `unitMove` | orgId, parentId | 자기 자신·자손·최상위(NULL) 거부 · 새 상위는 활성이어야 · 옮긴 뒤 `sort_order` = 새 형제 맨 뒤 · 이미 그 상위면 거부 |
| `unitReorder` | parentId, orgIds[] | 그 형제의 활성 전부가 와야 한다 |

- 잠금 경합(`IsLockContention`)·롤백(`SafeRollbackAsync`) 규율은 `SaveUserOrderAsync` 그대로.
- MainWindow: `case "orgTitleGet"` … `case "unitReorder"` 아홉 개. 회신은 `ReplyOnUi(reqId, new { ok, msg, orgTitle })`. **줄끝 `\r\r\n`·CRLF 24 규칙 유지**(바이트 스크립트로만 편집).
...
- 소속 행: **들여쓰기 트리**(깊이 × 16px, `▸` 없이 들여쓰기만) · 이름 · 재직자 수 배지 · ▲▼(같은 부모의 활성 형제끼리) · [이름변경] [상위 변경] [숨김]/[복구]. [상위 변경] 은 행 안에 드롭다운을 펼치지 않는다(좁은 폭에서 접힌다) — 작은 확인창에 `<select>` 하나: 후보 = 활성 조직 중 자기 자신·자기 자손을 뺀 것(들여쓰기 표기). 추가 입력줄의 상위 드롭다운 후보 = 활성 조직 전부, 기본값 없음(안 고르면 [추가] 가 꺼진다).
...
- **계약**(`tests/org-title-web.test.mjs`, jsdom): ① 마크업에 탭·행·버튼이 없다(부재) ② `#uaOrgTitle` 은 `uaAdminBar` 가 관리자·비순서편집일 때만 만든다 ③ 렌더는 회신 순서를 지킨다(정렬 없음) ④ 숨김 불가 행은 [숨김] 이 꺼지고 사유가 `title` 에 ⑤ 소속 트리 들여쓰기 = 깊이 ⑥ 상위를 안 고르면 [추가] 가 꺼진다 · [상위 변경] 후보에 자기·자손이 없다 ⑦ 쓰기 왕복은 `hostRequest` 한 길·회신 `orgTitle` 을 `otSeat` 한 곳이 앉힌다 ⑧ 닫을 때 `uaReload` 를 부른다 ⑨ 모듈 상태 변수 ≤ 5. 각 계약에 변이 ≥1 + 통제군.
- **호스트 계약**(`tests/org-title-host.test.mjs`, 소스 슬라이스): 아홉 명령이 `OpenAdminAsync` 를 지난다 · 숨김 조건 SELECT 가 쓰기와 같은 트랜잭션 · `unitAdd` 가 상위 없는 추가를 거부 · `unitMove` 에 순환 검사가 있고 NULL 상위를 거부 · 회신에 `orgTitle` · GRANT 세 파일이 INSERT, UPDATE 를 말한다 · DELETE 는 없다.
- **루프**(`tests/loop-org-title.mjs`, 실 위젯+실 DB, 접두 `zzJ`(직급)·`zzO`(조직)): 추가→개명→순서→숨김 거부(재직자 있음: zzU 직원을 잠시 그 직급/조직으로)→숨김→복구→상위 없는 추가 거부→상위 변경(소속자 따라옴)·순환 거부→정리. 마지막에 두 표에서 zz 행을 **UPDATE 로 숨김이 아니라 DELETE** 해야 하는데 앱 계정에는 DELETE 가 없다 → 루프는 관리자 계정(`TC_TEST_DB_ADMIN_PW`)으로 직접 지운다(휴지통 루프가 대역 관리자를 치우던 방식). 배포 게이트 규약(무작위 시드 5회)에 편입.

## 8. 하지 않는 것

- 영구 삭제(휴지통 종류 추가는 뒤로).
- 조직 트리 드래그 앤 드롭(▲▼ + [상위 변경] 으로 충분하다).
- 직급을 권한 판정에 쓰는 일(표 COMMENT 그대로: 표시·정렬 전용).
- `membersGet` 회신 모양 변경(드롭다운 소스는 그대로 그 회신이 정본).

## 9. 구현 순서

1. 이 문서 확정 — 소속 편집은 전부 관리창에서(상위 변경 포함 · 2026-09-18). 남은 확인: **숨김 조건**·**GRANT 적용 시점**.
...
**1. 복구는 순서를 맨 뒤로 보낸다(2026-09-18 구현).** `titleSetActive`/`unitSetActive` 의 복구가 `is_active=1` 만이 아니라 `sort_order = MAX+10`(소속은 형제 안 MAX)도 함께 쓴다. 순서 저장 두 명령이 **활성만** 10·20·… 으로 다시 매기므로, 옛 번호를 든 채 돌아온 행이 살아 있는 형제와 번호를 공유하게 된다 — 코드표 `SetCodeActiveAsync` 가 루프 I5 에서 잡았던 그 결함이다. §3.1·§4.2 의 "복구는 늘 됨" 은 그대로고, 돌아오는 자리가 맨 뒤라는 것만 더해진다.

**2. 1142 는 일반 DB 오류로 뭉개지 않는다(2026-09-18 실화면).** GRANT 미적용 상태에서 첫 쓰기를 눌러 보니 「처리하지 못했습니다(DB 오류).」 만 나왔다 — 관리자는 무엇을 고쳐야 하는지 알 수 없고, 휴지통 §11-2 의 사고(배포에서 GRANT 를 빠뜨림)가 정확히 이 번호로 죽는다. 아홉 쓰기 함수의 `MySqlException` 갈래에 `mex.Number == 1142 → GrantMissingMsg`(「DB 권한이 없습니다(1142) — 서버에 직급·소속 GRANT 를 적용해야 합니다(DEPLOY §0-5).」)를 앞세웠다. `loop-org-title` 은 이 문장을 보고 **판정 없음(exit 2)** 으로 끝난다(§6).

**3. 관리창 마크업 자리(2026-09-18 실화면).** `#orgTitleModal` 을 `#codeModal` 옆에 두었더니 「구성원 편집」에서 열었는데 그 창 **뒤**에 떴다 — `.overlay` 는 DOM 순서가 곧 쌓임 순서다. `#userEditModal` 뒤·`#trashModal` 앞으로 옮겼다(휴지통과 같은 이유로 같은 자리). 같은 화면에서 소속 탭의 추가 입력줄은 상위 드롭다운이 폭을 다 먹어 이름 칸이 0 에 가깝게 눌려 있었다 → `#otNewParent{flex:0 1 42%}` · `#otNewName{flex:1 1 0}`. 그리고 행에 버튼이 다섯이라 「기술개발총괄」이 잘려 `modal wide` 로 넓혔다.
