...
| 소속 조작 | 추가(부모 선택) · 개명 · 숨김/복구 · **형제 안 순서**(▲▼) · **부모 이동**(드롭다운, 순환 금지) | `org_unit` 은 PK=org_id·name UNIQUE·parent_id·sort_order·is_active. 개명은 번호 참조라 전파할 것이 없다. 부모 이동은 `parent_id` 한 컬럼 UPDATE — 자기 자신·자기 자손을 부모로 고르는 것만 막으면 된다 |
...
- **순환 금지**: 부모 이동 시 호스트가 새 부모의 조상 사슬을 걸어 올라가며 대상 자신이 나오면 거부한다(12행이라 메모리 순회). `parent_id` 를 NULL 로 두면 루트가 하나 더 생긴다 — 허용하되 확인창에서 말한다.
...
| `unitAdd` | name, parentId(NULL 허용) | 1~50자 · 중복 거부 · 부모는 활성이어야 · `sort_order` = 그 형제 최대+10 |
| `unitRename` | orgId, newName | 중복 거부 |
| `unitSetActive` | orgId, active | 숨김은 재직자 0명·활성 하위 0개일 때만 · 복구 시 부모가 숨김이면 거부("상위 조직을 먼저 복구하세요") |
| `unitMove` | orgId, parentId | 순환 거부 · 자기 자신 거부 · 부모 활성이어야 · 옮긴 뒤 `sort_order` = 새 형제 맨 뒤 |
...
- 소속 행: **들여쓰기 트리**(깊이 × 16px, `▸` 없이 들여쓰기만) · 이름 · 재직자 수 배지 · ▲▼(같은 부모의 활성 형제끼리) · [이름변경] [상위 변경] [숨김]/[복구]. [상위 변경] 은 행 안에 드롭다운을 펼치지 않는다 — 좁은 폭에서 접힌다. 작은 확인창(`confirmBox` 계열)에 `<select>` 하나: 후보 = 활성 조직 중 자기 자신·자기 자손을 뺀 것 + 「(최상위)」.
...
- **계약**(`tests/org-title-web.test.mjs`, jsdom): ① 마크업에 탭·행·버튼이 없다(부재) ② `#uaOrgTitle` 은 `uaAdminBar` 가 관리자·비순서편집일 때만 만든다 ③ 렌더는 회신 순서를 지킨다(정렬 없음) ④ 숨김 불가 행은 [숨김] 이 꺼지고 사유가 `title` 에 ⑤ 소속 트리 들여쓰기 = 깊이 ⑥ [상위 변경] 후보에 자기·자손이 없다 ⑦ 쓰기 왕복은 `hostRequest` 한 길·회신 `orgTitle` 을 `otSeat` 한 곳이 앉힌다 ⑧ 닫을 때 `uaReload` 를 부른다 ⑨ 모듈 상태 변수 ≤ 5. 각 계약에 변이 ≥1 + 통제군.
- **호스트 계약**(`tests/org-title-host.test.mjs`, 소스 슬라이스): 아홉 명령이 `OpenAdminAsync` 를 지난다 · 숨김 조건 SELECT 가 쓰기와 같은 트랜잭션 · 순환 검사 존재 · 회신에 `orgTitle` · GRANT 세 파일이 INSERT, UPDATE 를 말한다 · DELETE 는 없다.
- **루프**(`tests/loop-org-title.mjs`, 실 위젯+실 DB, 접두 `zzJ`(직급)·`zzO`(조직)): 추가→개명→순서→숨김 거부(재직자 있음: zzU 직원을 잠시 그 직급/조직으로)→숨김→복구→부모 이동(순환 거부 포함)→정리. 마지막에 두 표에서 zz 행을 **UPDATE 로 숨김이 아니라 DELETE** 해야 하는데 앱 계정에는 DELETE 가 없다 → 루프는 관리자 계정(`TC_TEST_DB_ADMIN_PW`)으로 직접 지운다(휴지통 루프가 대역 관리자를 치우던 방식). 배포 게이트 규약(무작위 시드 5회)에 편입.
...
1. 이 문서 확정(사용자 확인 — 특히 **부모 이동 포함 여부**·**숨김 조건**·**GRANT 적용 시점**).
