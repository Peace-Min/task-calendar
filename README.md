...
[![Version](https://img.shields.io/badge/Version-v0.7-3e5be0)](#-릴리스-내역)
...
- 📋 **보고서(2단 레이아웃)** — **일간/주간/커스텀** → 넓게 보기는 **좌 옵션·우 미리보기**, 좁으면 **⚙ 옵션 접기**. 과제별 `[과제명] : 시간` + 업무 불릿 초안 → 복사/저장. 포함 항목·**머리기호(개요)**·들여쓰기 선택. 커밋·할 일·일정 항목은 미리보기에서 바로 편집
- 🏢 **회사 보고 자동 작성** — 일간보고 자동 전송 · 주간보고 폼 자동 채움 · **주간을 netcus 일간보고 병합으로 생성**(내용 출처 = 캘린더 기록 ↔ netcus 일간보고) ([아래](#-회사-보고-연동-netcus))
...
- **주간보고 — 내용 출처 선택**: 주간/커스텀에서 **캘린더 기록**(이 앱 정리분) 또는 **netcus 일간보고 병합**(그 주 사이트에 제출한 일간보고를 읽어 **과제별 재취합 + 시간 합산**) 중 선택. netcus 병합본은 **읽기전용**(수정은 netcus 일간에서), 형식 벗어난 항목은 `[미분류]`에 사유와 함께 표시.
