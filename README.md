...
**바탕화면에 상주하는 오프라인 데스크톱 캘린더 위젯 — 과제별 git 커밋으로 일/주 보고서를 만들고, 사내 보고 시스템 자동 작성과 일정 시작 알림까지**
...
- 🧾 **보고서 자동화** — 과제별 git 커밋 → 작업일지 → 일간/주간 보고서 초안 → **회사 시스템 자동 작성/전송**.
...
- 🧩 **우측 패널 탭** — `[일정 상세 | 할 일 | Git 커밋 내역]`, 셋 다 **선택한 날짜**를 따라감. 좁은 폭에선 하단 시트
- ✅ **할 일(TODO)** — ★중요·완료 섹션·기한/기간, 캘린더에 **속 빈 링 칩**(일정 막대와 모양 구분, 색맹 안전)
- 🔧 **Git 커밋 → 작업일지** — 과제별 저장소에서 `내 커밋`을 끌어와 구조화 기록(단건·기간 일괄)
- 📋 **보고서(보고 유형 탭)** — **일간(오늘)/주간(이번 주)/커스텀 기간** → 과제별 `[과제명]:시간` + 업무 불릿 초안 → 복사/저장. 포함 항목(일정·할일·Git) 선택
...
| 우측 패널 전환 | `[일정 상세 / 할 일 / Git 커밋 내역]` 탭 — 모두 선택한 날짜 기준 |
| Git 커밋 가져오기 | **Git 커밋 내역 탭**의 `이 날 커밋` / `기간 일괄` (과제에 git 경로 연결, 위젯 전용) |
...
flowchart TB
  ...
  hostfn --> git["git CLI → 작업일지(source=git)"]
```
