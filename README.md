...
- 🖥️ **바탕화면 위젯** — 모든 앱 뒤(바탕화면 레이어)에 상주, 위치 고정(📌)·멀티 모니터 지원, 첫 실행 시 자동시작 여부 선택.
...
- 📌 **위치 고정** — 켜면 제자리 고정(이동 잠금), 끄면 상단 바 드래그로 이동 · 우하단 그립으로 크기 조절
...
| 위젯 이동 | 📌 고정 해제 후 상단 바 드래그 (고정 중엔 잠금) |
| 위젯 크기 | 우하단 그립 드래그 (고정/해제 모두) |
| 모니터 이동 | ⚙ → 다른 모니터로 이동 |
...
- **바탕화면 배치**: 창을 톱레벨 최하위로 둬 모든 앱 뒤(바탕화면 레이어)에 상주 — 렌더 안전·멀티모니터 자유. (초기 Progman reparent 방식은 일부 환경에서 WebView2 흰 화면 → 변경)
