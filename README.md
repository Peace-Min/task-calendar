...
- [x] **부팅 실패 시 [다시 시도]** + 자동 백오프(15·30·60초) · **낡은 클라이언트 쓰기 차단**(`schema_version` 비교 — 전량 교체만 거부)
- [ ] 알림 스킨(사용자 등록 이미지) · 주간/일간 타임라인 뷰
- [ ] 음력 명절(설·추석) 표시 · 회차별 반복 편집 · 미완료 할 일 롤오버

> 다음 릴리스에 무엇을 해야 하는지(배포 조건 P1·파일럿 계획)와 **왜 원래 계획과 달라졌는지**는 [docs/ROADMAP.md](docs/ROADMAP.md)가 정본입니다.
