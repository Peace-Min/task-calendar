...
- **일간 커밋 삭제**(항목2): git 커밋 라인에 삭제(✕) 추가 → 기존 `deleteCommitRow`(save+패널/그리드/보고서 재반영+토스트). git 라인만 노출(일정/할일은 캘린더에서).
...
- **근태·초과 위치**(항목6): 푸터 위 스트립 → **좌측 옵션 레일(날짜 아래)** `#rptAttendRail`로 이동. 저장/로직 불변.
