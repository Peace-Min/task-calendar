...
## 🆕 2026-06-22 — 버전 기준 재설정: v0.1 = 첫 배포(afac853), 패치노트 전면 갱신

이전에 v0.1을 `794cf24`(Linguist 보정)로 잡았으나, 사용자 확인 결과 **첫 배포는 그보다 이른 `afac853`(2026-06-11, "link Release download" — README 구축 직후 첫 Release)**. 그 시점엔 `openReport`/보고서 UI가 없었음(검증: afac853 HTML에 openReport/rptFrom 0건). 보고서·Git연동·테마 등은 모두 그 이후 추가.
- **태그 재설정**: `v0.1` → `afac853`(첫 배포). `v0.2` → 현재 HEAD. 중복·구체계 태그 **`v1.0.0` 삭제**(로컬·원격; afac853과 동일 커밋이라 v0.1로 대체).
...
요청: 사용자에게 알릴 패치노트 + 배포 버전 관리. (기준은 2026-06-22 재설정 참고 — v0.1=`afac853`)
